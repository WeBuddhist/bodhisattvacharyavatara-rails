#!/usr/bin/env python3
"""Upload a translation reader document to the reading-app API.


python3 translation_upload.py --instance-id <INSTANCE_ID> --dry-run   # preview payload
python3 translation_upload.py --instance-id <INSTANCE_ID> --author-person-id P12345678 --api-key <KEY>


Reads a reader JSON file produced by ``build_reader_json.py`` (with
``metadata``, ``content``, and ``verse`` fields) and POSTs a translation
payload to::

    POST {base-url}/v2/instances/{instance-id}/translation
...
DEFAULT_PREVIEW_OUTPUT = Path(__file__).resolve().parent / "payloads/preview.json"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "/v2/instances/<instance-id>/translation",
    )
    parser.add_argument(
        "--base-url",
    ...
    url = f"{args.base_url.rstrip('/')}/v2/instances/{args.instance_id}/translation"
