...
description: Segment an OCR-clean but under-segmented Tibetan commentary in 1-SOURCES/Commentaries into short, individually-referenceable blocks using botok-rs sentence boundaries only. Use when a commentary's text is one continuous run (or has overly long paragraphs) and needs breaking into citation-sized units before block IDs are applied. Runs AFTER format-commentary's OCR cleanup and BEFORE block-ID stamping and verse-context. Does not interpret, translate, or alter a single character of the source.
---

**Role:** Expert editor in classical Tibetan Buddhist commentary (`འགྲེལ་པ་`) structure and Obsidian markdown.

**Task:** Insert block boundaries into a Tibetan commentary so each block is a single botok-rs sentence — without adding, removing, reordering, or re-spelling any character of the source.

Sentence boundaries are detected entirely by **botok-rs** (`WordTokenizer` + `sentence_tokenize()`), a Rust-backed Tibetan word tokenizer that uses POS tags and a dictionary to identify terminal particles, clause boundaries, and verb endings. Each botok-rs sentence becomes one block. No rule-based post-processing is applied: there is no quote-frame, sa-bcad enumeration, ordinal-opener, verse-meter, or syllable-cap logic — segmentation is botok-rs and nothing else.
...
- **Granularity:** one botok-rs sentence per block — small enough that a downstream rail can cite the span it needs. The boundary is wherever botok-rs detects a sentence end (terminal particle, clause-boundary particle, or verb followed by a shad); nothing finer is imposed.
- **Verses and quotes:** these are NOT specially detected. A verse or quotation is broken wherever botok-rs places a sentence boundary inside it, like any other text. Splitting a stanza or separating an attribution onto its own block is a hand-review decision (see below), not something the script does.
...
```
[Stage 0]                   [Stage 1 — botok-rs]            [check]        [human]      [next skill]
preclean_commentary.py  →  segment_commentary.py  →  no-loss vs  →  approval  →  block-ID
(strip scaffolding,         (botok-rs sentence                source                     stamping
...
botok-rs must be installed before running Stage 1:

```bash
pip install git+https://github.com/OpenPecha/botok-rs.git
```

The `WordTokenizer` auto-downloads the `"general"` Tibetan dialect pack on first use (~30 MB, cached to `~/Documents/botok-rs/dialect_packs/`). Subsequent runs are fully offline.
...
| `segment_commentary.py`  | 1 | botok-rs sentence tokenization. The core of the skill. Output is the final segmented draft. |
...
**Stage 1 — botok-rs sentence tokenization**

`scripts/segment_commentary.py` uses **botok-rs** for sentence-level segmentation and does nothing else: each botok-rs sentence becomes one block.

Each input paragraph is tokenised with `WordTokenizer` (dictionary-backed, POS-tagged). The token stream is passed to `sentence_tokenize()`, which finds sentence boundaries at:
...
- **Clause-boundary particles** (`སྟེ་`, `ཏེ་`, `ནས་`, `ཞིང་`…) followed by punctuation.
- **Verbs** followed by punctuation.
- Short verb-less fragments are joined to their neighbour rather than left as isolated one-word blocks.

Structural lines are handled but never segmented: the frontmatter (`--- … ---`) is preserved verbatim, markdown headings (`#`…`######`) pass through on their own line, and blank-line-separated paragraphs are tokenised independently. If botok-rs returns no sentences for a paragraph, that paragraph is kept whole as a single block.
