...
For the LLM-facing version of these rules see `CLAUDE.md` Section 2 and Section 5. For blank file templates see `4-SYSTEM/templates/1-sources/`.
...
The commentary's own block IDs are always independent of the root verse IDs. The editorial note records which root verses a passage covers when that is not obvious from the transclusions. This feeds into `coarser_groupings:` in `2-RAILS/` packages.
...
|`-bo-acip`|ACIP transliteration|ACIP digital archive archive format|
