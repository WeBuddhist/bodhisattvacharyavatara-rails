#!/usr/bin/env python3
"""Lint commentary source documents against the API schema.

Commentary-only linter: rejects any file whose `file_type` is not
`commentary`. For root texts, editions, or translations use
`4-SYSTEM/scripts/linter-root-text/lint_text_input.py` instead.

On success: writes a valid JSON payload to output/<stem>.lint.json.
On failure: writes a failure report to output/<stem>.lint.errors.json.

Usage:
    python lint_commentary.py [path/to/commentary.md ...]
"""
...
from build import write_edition_output
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Validate a commentary source file against the TextInput / Edition schema."
    )
    parser.add_argument("paths", nargs="*", type=Path, help="Commentary markdown files to lint")
    ...
    for path in args.paths:
        ...
        if file_type != "commentary":
            print(
                f"ERROR {path}: this linter only accepts file_type: commentary "
                f"(got {file_type!r}). Use linter-root-text for other file types.",
                file=sys.stderr,
            )
            had_errors = True
            continue

        # Resolve root_text -> commentary_of, category_id before validation
        if not doc_info.get("commentary_of"):
            root_text_val = doc_info.get("root_text")
            if not root_text_val:
                print(f"  ERROR root_text: required for file_type 'commentary'", file=sys.stderr)
            else:
                ...
                if resolved_root:
                    try:
                        ...
                        if root_info:
                            ...
                            comm_of = root_info.get("text_id")
                            if comm_of:
                                doc_info["commentary_of"] = str(comm_of)
                            else:
                                print(f"  WARN root_text {resolved_root.name}: no text_id found to set commentary_of", file=sys.stderr)
