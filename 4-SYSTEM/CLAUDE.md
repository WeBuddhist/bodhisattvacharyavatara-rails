...
- `3-TRANSFORMATIONS/Day-Packages/bo/Chapter-*/[0-9]*.md` — Tibetan source packages
- `3-TRANSFORMATIONS/Day-Packages/bo/_TEMPLATE.md` and `_TERMBASE.md`
...
| `4-SYSTEM/` | **mostly no** — see exception below for skills and scripts |
...
**`4-SYSTEM/` exception — skills and scripts.** The default "no write" rule for `4-SYSTEM/` protects the *operational rules* (this `CLAUDE.md`, the folder READMEs / `About *.md` files, templates, and the citation-chain governance). The LLM **may** create and edit **skills** (`4-SYSTEM/Skills/<skill>/`, including `SKILL.md` and bundled scripts) and **standalone scripts** (`4-SYSTEM/Scripts/`) when explicitly asked to build or improve tooling. When it adds or changes a skill, it also updates `4-SYSTEM/Skills/SKILLS-CATALOG.md` to match. Everything else in `4-SYSTEM/` — the rules themselves — still requires a human contributor; do not edit this file or the folder READMEs to change a rule without explicit human instruction.
