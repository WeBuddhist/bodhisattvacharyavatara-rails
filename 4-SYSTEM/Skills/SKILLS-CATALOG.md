...
### `BCA-Verse-Context-Summary` **[exists]**
...
→ [`BCA-Verse-Context-Summary/SKILL.md`](BCA-Verse-Context-Summary/SKILL.md)
...
### `BCA-Verse-Distribution-Updater` **[exists]**
**Purpose:** Move a verse from one day to another in the Dalai Lama track schedule — updates the Verses and Studio Verse columns in `Tibetan-schedule-corrected.md`, rippling the shift through every day between the verse's old and new day, and renames the affected day files in the Dalai Lama plan folder to match. Same-chapter, boundary-verse moves only (a verse must be the first or last verse of its current day); cross-chapter moves and mid-day verses are rejected with an explanation rather than guessed. Day-file content is never touched, only the schedule table and file names.
**Inputs:** The verse to move (`chapter.verse`), the target Y.Day; defaults to `Tibetan-schedule-corrected.md` and the `Dalai Lama` plan folder.
**Outputs:** `Tibetan-schedule-corrected.md` updated in place for the affected rows; matching day files renamed (via `git mv` when possible) under `Plans/Dalai Lama/Chapter-<N> D..-D../`. Bundles a dry-runnable `scripts/shift_verse.py` (audit / plan / apply subcommands).
→ [`BCA-Verse-Distribution-Updater/SKILL.md`](BCA-Verse-Distribution-Updater/SKILL.md)
