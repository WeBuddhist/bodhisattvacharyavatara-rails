...
- **བསྡུས་དོན།** — Tibetan synthesis. Mirror the AI-Overview register: short sentences, leading claim first, cited bullets. This section is never composed by hand — it is generated by calling the `gemini_generate` tool (see [Tool call — བསྡུས་དོན། generation](#tool-call--བསྡུས་དོན-generation) below) and its returned text is inserted as-is.
...
## Tool call — བསྡུས་དོན། generation

The བསྡུས་དོན། section is never drafted directly by the model composing the page. It is produced by handing the model's own already-written sections to Google Gemini via MCP and inserting what comes back. This keeps the synthesis a genuine second-pass compression rather than a restatement invented in the same breath as the rest of the page.

**Tool:** `gemini_generate`, served by the `gemini-mcp` MCP server. Depending on how it is connected in the running session it may be exposed as `mcp__gemini-mcp__gemini_generate` or, when proxied through a device bridge, `mcp__remote-devices__gemini-mcp__gemini_generate` — call whichever name is actually present. If neither is available, **stop and report this**; do not fall back to composing བསྡུས་དོན། by hand.

**When to call it:** only after every other section (ལེགས་སྦྱར། through གནད་ཚིག) is finished and cited. Never before.

**How to call it:**
- `prompt` — concatenate the full, already-written text of ལེགས་སྦྱར། through གནད་ཚིག for this verse (citations included, verbatim, nothing paraphrased away), then append an instruction block equivalent to:

  > The text below is a compiled, fully-cited dossier for Bodhicaryāvatāra verse `<verse-id>` — root text, Zhenga's mchan-'grel, per-commentary explanations, stories, metaphors, scriptural quotations, main teaching points, and key terms. Write a Tibetan-language synthesis in the register of a Google AI Overview:
  > 1. Open with **ངོ་སྤྲོད་མདོར་བསྡུས།** — one or two sentences stating the settled reading the commentaries collectively support. Lead with the conclusion.
  > 2. Follow with **གནད་དོན་གཙོ་བོ།** — three to six short, scannable Tibetan bullets, one idea each.
  > 3. Use nothing but claims already present in the dossier below. Introduce no outside material and no parametric/background knowledge of your own.
  > 4. Every sentence and bullet must carry forward the exact citation(s) attached to the claim(s) it draws from — copy the citation markers `([[path#^block]])` from the dossier verbatim; do not invent, drop, or renumber them.
  > 5. If the dossier records a divergence between commentaries, preserve it — state both positions and mark ⚑, citing both sides — rather than flattening it into one voice.
  >
  > ---DOSSIER---
  > `<the concatenated sections>`

- `model` — leave at default unless the user has specified a Gemini model.

**After the call:**
- Insert the returned text verbatim as the body of བསྡུས་དོན།, replacing the `ངོ་སྤྲོད་མདོར་བསྡུས།` / `གནད་དོན་གཙོ་བོ།` placeholders in the template.
- Treat the returned text as a draft, not a finished section: the Stage 4 verification pass (step 19) must additionally confirm (a) every citation in the returned text actually appears in the dossier text that was sent, and (b) every claim in the returned text is traceable to something in that dossier. Hand-fix or re-run the call for any bullet that fails either check — a fabricated or unverifiable claim may never remain in the section.
...
10. **བསྡུས་དོན། is strictly derived, and generated, not hand-written.** Produce it only by calling `gemini_generate` (see [Tool call — བསྡུས་དོན། generation](#tool-call--བསྡུས་དོན-generation)) with every section written so far as its input, and inserting the returned text. Every claim in the result must be traceable to something already written in the sections above — this is a compression, not an expansion, and no new interpretive material enters here even via the model's own paraphrase.
...
16. **བསྡུས་དོན།** — do not compose this by hand. Concatenate every section written so far (ལེགས་སྦྱར། through གནད་ཚིག, citations included) into a prompt and call `gemini_generate` per [Tool call — བསྡུས་དོན། generation](#tool-call--བསྡུས་དོན-generation) to produce the AI-Overview-style synthesis strictly from that material. Insert the returned text; verify every citation and claim in it traces back to the dossier before accepting it.
...
- [ ] **བསྡུས་དོན།**: generated via `gemini_generate` (not hand-composed) from the concatenated sections above; AI-Overview style; lead claim first; bullets scannable; every claim and citation in the returned text verified against the dossier that was sent; ⚑ on splits cited both sides.
