...
└── 4-SYSTEM/             # guidelines, skills, templates, agent instructions
    ├── docs/             # methodology guidelines + human-facing how-to guides (this folder)
    ├── Skills/           # repeatable workflows (ingest, format, extract, translate, QA)
    ├── Templates/        # blank file templates for each frontmatter type
    ├── ⚡ Pipelines/      # end-to-end process docs (diagram + detail per flow)
...
- `docs/` — cross-cutting documentation: both the methodology guidelines that don't belong to any single layer (this file, [`why-rails.md`](why-rails.md), the per-text [`vault-annex.md`](vault-annex.md)) and the human-facing how-to guides for non-AI tasks (vault setup, sync troubleshooting, transcription workflows). Layer-specific rules (1-SOURCES, 2-RAILS, 3-TRANSFORMATIONS) live in each folder's own `About <FolderName>.md`.
...
- `⚡ Pipelines/` — end-to-end process docs showing how skills and scripts chain into a flow; one diagram + one detail doc per pipeline. See `⚡ Pipelines/README.md`.
...
- [ ] Copy `4-SYSTEM/docs/` from a prior vault (this file, `why-rails.md`, and the how-to guides) — no text-specific edits needed
...
- [ ] Fill in `4-SYSTEM/docs/vault-annex.md` with text-specific conventions
...
The `docs/` and `Skills/` folders are deliberately text-agnostic so they can be carried across vaults without modification. The only files that need per-text adaptation are `README.md`, `CLAUDE.md`, and `vault-annex.md`.
...
2. `4-SYSTEM/docs/why-rails.md` — why the methodology works (specialist-pair and Wikipedia analogies)
3. `4-SYSTEM/docs/0-VAULT-Structure.md` — this file: the architecture
...
8. `4-SYSTEM/docs/vault-annex.md` — text-specific conventions
