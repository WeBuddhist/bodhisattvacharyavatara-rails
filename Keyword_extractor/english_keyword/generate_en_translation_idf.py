...
Every word is also tagged with the Obsidian block ID(s) (e.g. "1-1", "9-31-0")
of the verse/heading block(s) it occurs in, so a term can be traced back to
its source.
...
Default source (edit SOURCE_PATHS to change):
    sanskrit-english/english/bca-english-plain-zeroshot.md

Output is written next to this script, e.g.:
    output/bca-english-plain-zeroshot_keyword.json   (word + reference ids)
    output/bca-english-plain-zeroshot_scores.md      (ranked | Score | Keyword | table)
"""
...
# ---------------------------------------------------------------------------
# Configuration — edit these paths to change sources
# ---------------------------------------------------------------------------
...
# Words with IDF <= this are common in general English (function words and
# common content words) and are dropped from the combined keyword output.
...
def extract_blocks(text: str) -> list[tuple[str | None, str]]:
    """
    Split a (frontmatter-stripped) vault markdown file into (block_id, text)
    pairs. Splits on blank lines and attaches each run of paragraphs to the
    block ID it ends with, so a word found inside that text can be traced
    back to its source block.
    """
...
def count_tf(path: pathlib.Path) -> tuple[dict[str, int], dict[str, list[str]]]:
    """
    Returns (counts, word_ids):
      counts:   word -> total occurrence count in the file
      word_ids: word -> block IDs (document order, deduped) where it occurs
    """
    ...
    for block_id, block_text in blocks:
        tokens = [t.lower() for t in _TOKEN_RE.findall(_clean_markup(block_text))]
        seen_in_block: set[str] = set()
        for w in tokens:
            if w in STOPWORDS or len(w) <= 2:
                continue
            counts[w] = counts.get(w, 0) + 1
            if block_id is not None and w not in seen_in_block:
                word_ids.setdefault(w, []).append(block_id)
                seen_in_block.add(w)

    return counts, word_ids
...
def build_rows(tf: dict[str, int], word_ids: dict[str, list[str]]) -> list[dict]:
    total = sum(tf.values())
    rows  = []
...
def _ranked_english_words(
    ...
    def word_idf(w: str) -> float:
        for s in stems:
            row = comp[s]["row_map"].get(w)
            if row is not None:
                return row["idf"]
        return IDF_UNKNOWN
...
def write_combined_json(
    ...
    def word_ref_ids(w: str) -> list[str]:
        ...
        seen: set[str] = set()
        for s in stems:
            for bid in comp[s]["row_map"].get(w, {}).get("ids", []):
                if bid not in seen:
                    seen.add(bid)
                    ids.append(bid)
        return ids
...
def render_md(rows: list[dict], source: pathlib.Path) -> str:
    ...
    L += [
        ...
        "",
        "**Reference IDs** — the Obsidian block ID(s) (`^1-1`, `^9-31-0`, etc.) of every",
        "verse/heading block where the word occurs.",
        "",
    ...
    for i, r in enumerate(top, 1):
        b = band(r["score"])
        ids_str = ", ".join(f"^{bid}" for bid in r["ids"][:8]) if r["ids"] else "-"
    ...
    L += [
        ...
        "",
        "| Rank | Word | Count | TF-IDF | IDF | Band | Reference IDs |",
        "|------|------|-------|--------|-----|------|---------------|",
    ]
    for i, r in enumerate(rows, 1):
        b     = band(r["score"])
        score = f"{r['score']:,.2f}"
        ids_str = ", ".join(f"^{bid}" for bid in r["ids"]) if r["ids"] else "-"
        L.append(f"| {i} | **{r['word']}** | {r['count']} | {score} | {r['idf']} | {b} | {ids_str} |")
...
def build_comparison(sources: list[pathlib.Path]) -> dict:
    ...
    result = {}
    for src in sources:
        tf, word_ids = count_tf(src)
        rows = build_rows(tf, word_ids)
        result[src.stem] = {
            "rows":    rows,
...
def main() -> None:
    ...
    sources = [p.resolve() for p in SOURCE_PATHS]
