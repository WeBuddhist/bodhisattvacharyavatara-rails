...
def get_text_segments(text_id: str) -> dict:
    ...
    for seg_doc in segment_docs:
        ...
        content_map[uuid_str] = seg_doc["content"]
    ...
    for seg in flat_segments:
        segments.append({
            "segment_id": seg["segment_id"],
            "segment_number": seg["segment_number"],
            "content": content_map.get(seg["segment_id"], "MISSING"),
            "tags": ""
        })
...
def save_csv(result: dict, output_path: str):
    with open(output_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["text_id", "segment_number", "segment_id", "content", "tags"])
...
def save_md(result: dict, output_path: str):
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write("| segment_number | segment_id | content | tags |\n")
        f.write("|---|---|---|---|\n")
        for seg in result["segments"]:
            content = seg["content"].replace("|", "\\|").replace("\n", " ")
            f.write(f"| {seg['segment_number']} | `{seg['segment_id']}` | {content} | {seg['tags']} |\n")
    print(f"Saved to {output_path}")
...
def main():
    ...
    if args.output:
        ...
        print("\ntext_id,segment_number,segment_id,content,tags")
        for seg in result["segments"]:
            print(f"{result['text_id']},{seg['segment_number']},{seg['segment_id']},{seg['content']},{seg['tags']}")
