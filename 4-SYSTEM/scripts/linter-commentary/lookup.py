...
def search_bdrc_work(title):
    """Search BDRC Elasticsearch for a work by title. Returns (work_id, warn)."""
    index_line = json.dumps({"index": "bdrc_prod"})
    query_line = json.dumps({
        "from": 0, "size": 10,
        "query": {
    ...
    try:
        ...
        return None, f"BDRC work search failed for {title!r}: {exc}"
    hits_list = envelope.get("responses", [envelope])[0].get("hits", {}).get("hits", [])
    for hit in hits_list:
        types = hit.get("_source", {}).get("type", [])
        hid = hit.get("_id", "")
        if "PartTypeText" in types or hid.startswith("WA"):
            return hid, None
        for merged_id in hit.get("_source", {}).get("merged", []):
            if merged_id.startswith("WA"):
                return merged_id, None
    return None, f"no work found in BDRC for title {title!r}"


def fetch_bdrc_work_titles(work_id):
    """Fetch TTL for a BDRC work. Returns ({"pref": {...}, "alt": [...], "work_id": ...}, warn)."""
    try:
        from rdflib import Graph, URIRef
        from rdflib.namespace import SKOS
    except ImportError:
    ...
    try:
        ...
        return None, f"TTL fetch failed for {work_id}: {exc}"
    g = Graph()
    try:
        g.parse(data=ttl_data, format="turtle")
    except Exception as exc:
        return None, f"TTL parse failed for {work_id}: {exc}"
    subject = URIRef(f"http://purl.bdrc.io/resource/{work_id}")
    pref, alt = {}, []
    for label in g.objects(subject, SKOS.prefLabel):
        lang = str(label.language or "")
        if lang and str(label).strip():
            pref[lang] = str(label).strip()
    for label in g.objects(subject, SKOS.altLabel):
        lang = str(label.language or "")
        if lang and str(label).strip():
            alt.append({lang: str(label).strip()})
    if not pref and not alt:
        return None, f"no title labels found in TTL for {work_id}"
    return {"pref": pref, "alt": alt, "work_id": work_id}, None
