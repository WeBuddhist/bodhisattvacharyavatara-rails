...
```
* SECTION TITLE ^toc-1
   * SUBSECTION TITLE ^toc-1-1
      * SUB-SUBSECTION TITLE ^toc-1-1-1
      * ... ^toc-1-1-2
   * SECOND SUBSECTION ^toc-1-2
* SECOND SECTION ^toc-2
```
...
- The TOC block is preceded by a `## དཀར་ཆག / Table of Contents` heading
...
Files that already contain a `## དཀར་ཆག` section with `^toc-*` block IDs
and Obsidian wiki links `[[#^id|text]]`. The script strips the wiki links
and reformats with 3-space indentation.
...
- Bracket numbers: `༡༽`, `༢༽`, `༣༽`, …
- Bracket letters: `ཀ༽`, `ཁ༽`, `ག༽`, `ང༽`, …
- Decimal labels: `༡.༡`, `༡.༢`, `༢.༡`, …
- Section closers: trailing `ལོ།།`, `ལ།`, `།།`
