#!/usr/bin/env python3
"""
Renumber Obsidian block-reference IDs (^N-M) on every content segment of a
Bodhicaryavatara-style commentary file, restarting the counter at ^N-1 for
each chapter.
...
(see below), bounded by blank lines. This includes:
  ...
  - multi-line verse/stanza blocks (`>` blockquote lines) -- a whole stanza
    gets exactly ONE id, appended to its last line, same as any paragraph
  - a short lead-in line glued directly to a following stanza with no
    ...
    and get one id together, per this vault's segmentation convention
...
IDs already sitting on chapter/sub-heading lines (like `^I-0`, `^1-0`) are
never touched -- only content-run segments are numbered.

Any pre-existing block id at the end of a segment's last line is stripped
and replaced with the freshly computed one, so this script is safe to
re-run on a file that already has (possibly stale or partial) block ids.

Usage:
    python3 update_block_ids.py <path-to-tagged-markdown-file>
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Usage: python3 update_block_ids.py <path-to-tagged-markdown-file>", file=sys.stderr)
        sys.exit(1)
